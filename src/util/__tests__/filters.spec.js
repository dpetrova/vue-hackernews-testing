...
//testing timeAgo filter
describe('timeAgo filter', () => {
  Date.now = () => new Date('2020') //mock Date.now to always return the same date
  const unixTime = Date.now() / 1000 //convert Date.now to UNIX time

  //helper functions to convert timestamp into seconds
  //(UNIX timestamps are the number of seconds that have passed since 1970)
  ...
  test('returns singular minute', () => {
    //assert that timeAgo returns the correct value when called with a timestamp from one minute ago
